...
function getDesktopRequestToken(req) {
  return (
    req.headers['x-music-desktop-token'] ||
    req.query?.desktopToken ||
    req.query?._desktop_token ||
    req.body?.desktopToken ||
    req.body?._desktop_token ||
    ''
  )
}
...
async function constructServer(moduleDefs) {
    ...
    if (req.path !== '/' && !req.path.includes('.')) {
      ...
      res.set({
        ...
        'Access-Control-Allow-Headers':
          'X-Requested-With,Content-Type,X-Music-Desktop-Token',
  ...
  if (process.env.DESKTOP_API_TOKEN) {
    app.use((req, res, next) => {
      const requestToken = getDesktopRequestToken(req)
      if (timingSafeEqualString(requestToken, process.env.DESKTOP_API_TOKEN)) {
        next()
        return
      }
